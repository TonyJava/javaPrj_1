...
public class MessageDAOImpl implements MessageDAO{
	...
	public List<Message> findAllMessagee(Employee employee,Page page) {
		Connection conn = DBConnection.getConnection();	//获得连接对象
		String findSQL = "select *from  tb_message where messageID in "
				+ "	(select messageID from tb_messagepermission "
				+ "where employeeID=?) order by  "
				+ "publishTime desc limit ?,?";
		PreparedStatement pstmt = null;					//声明预处理对象
		ResultSet rs = null;
		...
		try {
			...
			pstmt.setInt(2, page.getBeginIndex());		//查询起始点
			pstmt.setInt(3, page.getEveryPage());		//查询记录数
